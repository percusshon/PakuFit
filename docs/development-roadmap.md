# 開発ロードマップ

## Phase 0: docs/設計

- 本リポジトリを設計ドキュメントのみで構成
- MVP要件、セーフティ、RLS、DB、提携方針を定義

## Phase 1: Next.js + Supabase scaffold

- 認証基盤の雛形
- プロジェクト構成、CI基盤、環境変数設計
- 画像保存と日次集計の基本テーブル整備（設計通り）

## Phase 2: 食事記録MVP

- 写真/テキスト入力画面
- 食事一覧・日次一覧
- 目標登録画面（体重管理/バランス重視/高たんぱくなど）

## Phase 3: カロリー/PFC概算

- 推定値を概算表示
- 補正UIの実装
- 日次サマリーと残量表示

## Phase 4: 次の食事候補提案

- ルールベース候補生成を導入
- 候補理由表示と安全ラベル
- 「候補」表現を徹底

## Phase 5: 写真AI連携

> 状態: 概算機能の基盤を実装済み（モック既定 + 実プロバイダ差し替え可）。実プロバイダは OpenAI vision（既定 gpt-4o-mini）と Anthropic vision を選択可能（`PAKUFIT_VISION_PROVIDER` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY`、未指定はキーの有無で自動判定・OpenAI優先、失敗時はモックfallback）。精度ログ観測の土台として、写真AI概算を反映した保存時に AI元概算と最終保存値を `photo_estimate_logs`（RLS本人のみ・追記専用、migration 0006）へ記録する（補正率の観測が可能）。外部送信は明示オプトイン（同意ONのときだけ送信・サーバ側強制、未同意はモック）で、プライバシー文言（総称「外部AIサービス」/保持・学習は規約参照/画像非保存）を `/privacy` に反映済み。コストの最終確定・送信先規約の確認・法務レビュー、観測値の集計（`scripts/estimate-accuracy.sql`）の可視化は継続課題。

- 画像解析サービス接続（設計外部API）
- 推定精度ログと補正率の観測
- 品質が低いケースで補正導線最適化

## Phase 6: バーコード/JANコード

> 状態: ブラウザ標準 `BarcodeDetector` のみで JAN(EAN-13/8 等) 読取UIを実装済み（`components/barcode-scanner.tsx`、未対応端末は機能非表示で案内のみ、外部ライブラリ不使用）。読み取ったJANは食事名へ反映できる。商品DB照合（JAN一致時の候補自動補完）は外部データソース/契約が必要なため未着手。

- 条件分岐で手入力/バーコード入力を共存
- JAN一致時の候補自動補完

## Phase 7: コンビニ商品DB

- 商品DB同期、在庫・価格・地域条件の導入
- 提携先情報の表示分離（広告表示と非広告表示）

## Phase 8: 提携/広告/収益化

- 説明付き広告導線の追加
- 利用者透明性（提携表示、広告表示）を担保

## Phase 9: モバイルアプリ化

> 状態: PWA 基盤を実装済み（manifest/icon/SW・インストール可能化・オフラインフォールバック、commit 3f2e496）。追加で PNG アイコン（192/512・maskable・apple-touch-icon=`app/apple-icon.png`）整備、写真入力にカメラ直接撮影導線を追加済み。高度なオフライン対応は未着手。

- PWA/ネイティブ移行検討
- カメラ連携最適化
- オフライン考慮

## 進行指標（共通）

- ユーザー補正率と、推定利用満足度の観測
- 安全表示の表示率（必要時に表示されているか）
- 相談誘導文言の適切率

## 次の未確認事項

- 公開規制/地域法、個人情報保護法への詳細照合
- パートナー提携時の金銭的契約条件
- 法務最終レビュー時点での用語統一
