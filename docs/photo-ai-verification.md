...
2. `/meals/new` → 「カメラで撮影」または「ライブラリから選択」で料理写真を選ぶ。
3. 「AIで概算する」を押す。
4. 期待結果:
   - 概算結果に **「モック推定」バッジが出ない**（= 実プロバイダが使われた）。
   - 栄養値・推定ラベル・信頼度が表示される。
   - キー誤り/レート超過時は自動でモックにフォールバックし「AI接続に失敗…」注記が付く（保存は止まらない）。
5. 「この概算をフォームに反映」→ 値を必要に応じて補正 → 保存。
6. 精度ログ確認（開発者向け）: `scripts/estimate-accuracy.sql` を Studio/psql で実行し、
